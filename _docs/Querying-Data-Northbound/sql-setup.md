...
AnyLog uses a SQL dialect that routes queries across the network. See [Queries](../Querying%20Data%20(Northbound)/queries/) for the full reference. Key points:
...
The Operator service can be configured to archive processed JSON and SQL files. See [Background Services — Operator](../Network-Services/background-services.md#operator-service) for `archive_json` and `archive_sql` options.
